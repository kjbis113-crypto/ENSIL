...
## #9 배포본(https)에서 로컬 브릿지 연결 차단 — 2026-08-08

**증상** — https://ensil.vercel.app 에서는 물리연동이 "대기"로 남음.
로컬 실행(dev/preview)에서는 같은 브릿지에 정상 연결·실보드 데이터 수신 확인.

**원인** — https 페이지에서 비보안 `ws://localhost:7777` 연결은 브라우저의
혼합 콘텐츠(mixed content) 정책에 걸린다. localhost 예외는 http 요청에는 적용되지만
WebSocket에는 일관되게 적용되지 않음 (브릿지 로그로 미연결 확인).

**조치** — 전시·하드웨어 데모는 어차피 로컬 실행이 원칙(plan.md §14)이므로 구조 변경 없음.
- 하드웨어 데모: `npm run dev` 또는 `npx serve dist` (로컬 http → ws 문제 없음)
- 배포본은 온라인 열람용. 굳이 배포본+하드웨어를 묶으려면
  `chrome --allow-running-insecure-content` 플래그 또는 브릿지에 wss(TLS) 추가 필요.

**교훈** — "웹은 어디서든, 하드웨어는 로컬에서"가 기본 전제. 전시 체크리스트에 반영.
